...
void GameClient::keepTrackOfLinearLocalPositoins(const Player& player, const sf::Clock& clock, const sf::Int32& offset)
{
	...
	if (linear_local_positions.size() >= lin_num_msg) linear_local_positions.pop();
...
{
	if (linear_network_positions.size() >= lin_num_msg) linear_network_positions.pop();
...
void GameClient::keepTrackOfQuadraticLocalPositoins(const Player& player, const sf::Clock& clock, const sf::Int32& offset)
{
	...
	local_message.time = (float)getCurrentTime(clock, offset);
	//
	if (quadratic_local_positions.size() >= quad_num_msg) quadratic_local_positions.pop_back();
...
{
	if (quadratic_network_positions.size() >= quad_num_msg) quadratic_network_positions.pop_back();
