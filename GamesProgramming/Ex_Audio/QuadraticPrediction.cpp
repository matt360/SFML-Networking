...
inline float QuadraticPrediction::lerp(float start, float end, float time)
{
	return start * (1.0f - time) + time * end;
}

sf::Vector2f QuadraticPrediction::lerp(const sf::Vector2f& start, const sf::Vector2f& end, const float& time)
{
	sf::Vector2f temp;
